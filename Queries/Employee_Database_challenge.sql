...
-- Create unique titles table for only current retiring employees
-- Drop table if necessary
DROP TABLE current_unique_titles;
-- Use Dictinct with Orderby to remove duplicate rows
SELECT DISTINCT ON (e.emp_no) e.emp_no, e.first_name, e.last_name,
	t.title, t.from_date, t.to_date
...
-- Find count of current employees
SELECT count(emp_no) FROM current_unique_titles;
